...
# serves the built static site (output/) via Rack::TryStatic. This is the exact
# path Heroku uses (Procfile: `puma -p $PORT -e $RACK_ENV`) -- distinct from
# server_test.rb, which tests the Bridgetown dev preview server.
#
...
# which now runs the esbuild frontend bundle + `bridgetown build` -- so this
# test covers the real deploy build path (it fails if rake/node/esbuild isn't
# available, which would silently break the deploy).
BUILD_OK = system('bundle exec rake assets:precompile', out: File::NULL, err: File::NULL)

class ProductionTest < Minitest::Test
  PORT = 4569
  LOG  = '/tmp/bt_test_puma.log'

  def setup
    assert BUILD_OK, 'deploy build (rake assets:precompile) failed; cannot test the serving stack'
  ...
  def test_serves_compiled_asset
    # esbuild fingerprints the bundle name, so discover the real URL from the
    # page rather than assuming a fixed path.
    css_path = get('/').body[%r{href="(/_bridgetown/static/index\.[^"]+\.css)"}, 1]
    refute_nil css_path, 'could not find the fingerprinted CSS bundle link in the page'
    res = get(css_path)
